...
void conjugate_gradient(int n, matrix *A, matrix *M, FLOAT *b, FLOAT *x, int maxiter, FLOAT umbral, int step_check, int *in_iter)
{
    ...
    /*
        This is also awful, but need some way of looping til we hit the precision limit,
        then we switch to pairs
    */
    if(!A->useTail) {
        while ((iter < maxiter) && (tol > umbral) && !A->useTail) {
            // alpha = (r,z) / (Ap,p)
            ...
            if(residual < ManSeg::AdaptivePrecisionBound)
            {
                printf("hit precision bound at: iter=%d, in_iter=%d, residual=%e\n", iter, *in_iter, residual);
                A->useTail = true;
                break;
            }
        }
