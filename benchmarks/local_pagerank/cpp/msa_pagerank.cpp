...
public:
    ...
    void iterate_fullSet(double d, TwoSegArray<false>& prevPr, TwoSegArray<true>& newPr, int outdeg[], double* contr)
    {
        int src, dest;
        for(int i = 0; i < numEdges; ++i)
        {
...
void pr(SparseMatrix* matrix, std::chrono::time_point<std::chrono::_V2::system_clock, std::chrono::nanoseconds>& tmStart, string& inputFile)
{
    ...
    */
    {
        matrix->iterate_fullSet(d, x.heads, y.pairs, outdeg, contr);
        ...
        x.precisionSwitch();
        y.precisionSwitch();
        // after this point, x.(heads/pairs) and y.(heads/pairs) are deleted
...
/*      
            This stuff includes writing to accumulator
*/
