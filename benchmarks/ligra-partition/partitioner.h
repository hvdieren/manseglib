...
#else // no NUMA, no CILK
#define loop(vname,part,PerNode,code) {                                 \
	for(intT vname=0; vname < part.get_num_elements(); ++vname) {  \
		code;                                                   \
      }                   						\
}
#endif
...
#define map_partition(vname,part,code)					\
    {									\
	intT perNode = (part).get_num_per_node_partitions();		\
	parallel_for_numa( intT _i=0; _i < num_numa_node; ++_i ) {	\
	    parallel_for( intT _j=0; _j < perNode; ++_j ) {		\
		intT vname = _i * perNode + _j;				\
		code;							\
	    }								\
	}								\
    }
