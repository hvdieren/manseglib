...
#define DDEGREE 0
#if DDEGREE
        intT * check= new intT [GA.n];
        parallel_for(intT i=0; i<GA.n; i++)
            check[i]=GA.V[i].getInDegree();
        mysort(&check[0],&check[GA.n],PairSort(GA.n));
        intT c=1;
        for (intT i=1; i<GA.n; i++){
           if (check[i]!=check[i-1]) 
                c++;
        }
        intT *DegreeN = new intT [c];
        ...
        assert(a==c);
        delete [] check;
        cerr<<"Parition 	In-Degree 	Frequency"<<endl;	
        for (intT i =0; i <coo_part; i++){
           for (intT j=0; j<c; j++){
             intT b=0;
             for (intT k=coo_partition.start_of(i); k<coo_partition.start_of(i+1);k++){
                 if (GA.V[k].getInDegree()==DegreeN[j])
                      b++;
             }
             if(b!=0){
             cerr<<i<<" 	"<<DegreeN[j]<< " 	"<<b<<endl;
             }
           }
        }
        delete [] DegreeN;
...
#if SIZE
           for(intT i=0; i<coo_part; i++){
               intT VS=coo_partition.start_of(i+1)-coo_partition.start_of(i);
               intT ES=localEdgeList[i].get_num_edges();
               cerr<<"VERTER_CHUNCK: "<<VS<<" and EDGE: "<<ES<<endl;
           }
           abort();
#endif
