...
#cd /home/jsun/sjw/ligra-partition/coo-csr
    ...
    mkdir -p $OUTDIR
      ...
      if [ "$g" == "Yahoo_mem_undir_MinEdges_b" ]; then
       ...
       echo Algorithm:${f} >>"$outfile" 2>&1
             ...
             for p in $partitions;do
               ...
               LD_PRELOAD="./bin/interposer_cilk.so" numactl ./${f} -b -o -c ${p} -rounds 20 -r ${v} ${data}/${g} >> "$outfile" 2>&1
            done
          done
       done
     done 
     ...
     outfile=${OUTDIR}/${wg}_Hilbert.txt
      if [ "$g" == "Yahoo_mem_undir_MinEdges_wgh_b" ]; then
    	wv="1472832"
      elif [ "$g" == "orkut_undir_MinEdges_wgh_b" ]; then
    	wv="715576"
      elif [ "$g" == "twitter_dir_MinEdges_wgh_b" ]; then
    	wv="38054692"
      elif [ "$g" == "friendster_dir_MinEdges_wgh_b" ]; then
    	wv="103509270"
      elif [ "$g" == "RMAT27_dir_MinEdges_wgh_b" ]; then
    	wv="64349498"
      elif [ "$g" == "LJ_dir_MinEdges_wgh_b" ]; then
    	wv="518065"
      elif [ "$g" == "USAroad_undir_MinEdges_wgh_b" ]; then
    	wv="6577110"
      elif [ "$g" == "power_dir_MinEdges_wgh_b" ]; then
    	wv="62507412"
      else
    	wv="100"
      fi
        ...
        echo Algorithm:${wf} >>"$outfile" 2>&1
            ...
            export CILK_NWORKERS=${wt}
              echo Vertex:$wv>>"$outfile" 2>&1
               LD_PRELOAD="./bin/interposer_cilk.so" numactl ./${wf} -b -o -c 384 -rounds 20 -r ${wv} ${wghdata}/${wg} >> "$outfile" 2>&1
            done
       done
   done
